...
/// Failure domain levels for placement. Ordered from most-correlated
/// (`Disk`) to least-correlated (`Region`); a pool that asks for
/// placement at level X means shards must land on distinct values at
/// that level. Adding levels is a breaking change to serialized topology
/// — keep appended enum discriminants stable.
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
pub enum FailureDomain {
    /// Individual disk failure
    Disk = 0,
    /// Server/OSD process failure — keep multiple OSDs on the same box from
    /// being treated as independent failures.
    Node = 1,
    /// Physical host — below rack, above OSD process. Multiple `Node`s
    /// (OSD processes) may share a `Host` but fail together on power loss.
    Host = 5,
    /// Rack / power domain failure
    Rack = 2,
    /// Datacenter failure
    Datacenter = 3,
    /// Availability zone / row — between datacenter and region. Matches
    /// cloud AZ semantics and on-prem row-level power/cooling separation.
    Zone = 6,
