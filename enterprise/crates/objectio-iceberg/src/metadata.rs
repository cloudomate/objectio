...
fn apply_single_update(metadata: &mut Value, update: &crate::types::TableUpdate) -> Result<()> {
    ...
    match update {
        ...
        TableUpdate::SetSnapshotRef { ref_name, rest } => {
            ...
            // The Iceberg spec keeps `current-snapshot-id` as a denormalized
            // pointer to the snapshot id of the `main` branch ref. Readers
            // like PyIceberg rely on it — if we only update `refs.main`,
            // `current_snapshot()` returns None.
            if ref_name == "main"
                && let Some(snap_id) = rest.get("snapshot-id").and_then(Value::as_i64)
            {
                metadata["current-snapshot-id"] = Value::from(snap_id);
            }
        }
